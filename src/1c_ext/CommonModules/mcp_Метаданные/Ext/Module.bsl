...
Процедура ЗаполнитьТаблицуИнструментов(ТаблицаИнструментов) Экспорт
	...
	// Дополняем таблицу динамическими инструментами из регистра сведений
	ЗаполнитьДинамическиеИнструменты(ТаблицаИнструментов);
...
//
// Заполняет таблицу инструментов из регистра сведений mcp_ДинамическиеИнструменты
//
// Параметры:
//  ТаблицаИнструментов - ТаблицаЗначений - таблица инструментов для дополнения
//
Процедура ЗаполнитьДинамическиеИнструменты(ТаблицаИнструментов)
	...
	Попытка
		...
		"ВЫБРАТЬ
		|	ДИ.Имя КАК Имя,
		|	ДИ.Описание КАК Описание,
		|	ДИ.СхемаПараметров КАК СхемаПараметров
		|ИЗ
		...
		Пока Выборка.Следующий() Цикл
			НоваяСтрока = ТаблицаИнструментов.Добавить();
			НоваяСтрока.ИмяОбработкиКонтейнера = "_ДинамическийИнструмент";
			НоваяСтрока.Имя = Выборка.Имя;
			НоваяСтрока.Описание = Выборка.Описание;
			НоваяСтрока.СхемаПараметров = Выборка.СхемаПараметров;
		КонецЦикла;
	Исключение
		// Регистр ещё не создан или ошибка — пропускаем
	КонецПопытки;
